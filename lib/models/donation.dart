...
import 'package:google_maps_flutter/google_maps_flutter.dart' show LatLng; // Import LatLng
...
class DonationLocation {
  ...
  final String donorName; // Name of the donor
  ...
  factory DonationLocation.fromFirestore(Map<String, dynamic> doc) {
    ...
    return DonationLocation(
      ...
      expiryDate: (doc['expiryDate'] as Timestamp).toDate().toLocal().toString(), // Convert Firestore Timestamp to a readable string
      status: doc['status'] ?? 'available',
      donorId: doc['donorId'] ?? '', // Get donor ID from the Firestore document
      donorName: doc['donorName'] ?? 'Anonymous', // Get donor name from the Firestore document
    );
  }
